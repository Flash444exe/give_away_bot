...
db.execute("create table if not exists give_role (role_id integer)")
db_member.execute("create table if not exists member_give (members_id integer)")
cr = db.cursor()
...
async def role_member(interaction: discord.Interaction, الرتبة: discord.Role, الوظية:Literal["اضافة","ازالة"]):
    ...
    cr.execute("select role_id from give_role")
    ...
    try:
        if user.id == owner:
            if الوظية == "اضافة":
                if الرتبة.id in result:
                    ...
                    cr.execute(f"insert into give_role(role_id) values({الرتبة.id})")
        ...
        await interaction.response.send_message("يبدو ان هناك خطأ تأكد انك ادخلت كل شيئ بشكل صحيح")
...
async def giveaway(interaction: discord.Interaction, الجائزة: str, الوصف: str, المدة: int, نوع_المدة: Literal["ثواني", "دقائق", "ساعات", "ايام"], عدد_الفائزين: int):
    cr.execute("select role_id from give_role")
    cr_member.execute("delete from member_give")
    db_member.commit()
    ...
    try:
        if roless in interaction.user.roles:
            if عدد_الفائزين >= 1:
                ...
                embed.set_thumbnail(
                    url=interaction.guild.icon._url
                )
                ...
                async def button_callback(interaction: discord.Interaction):
                    if interaction.user not in member:
                        ...
                        await interaction.response.send_message(f"لقد انضممت للجيف اوي يا {interaction.user.mention}", ephemeral=True)
                        cr_member.execute(f"insert into member_give(members_id) values({interaction.user.id})")
                        db_member.commit()
                    else:
                        remove_button = discord.ui.Button(label="خروج", style=discord.ButtonStyle.red)
                        remove_view = discord.ui.View()
                        remove_view.add_item(remove_button)
                        async def remove_callback(interaction: discord.Interaction):
                            if interaction.user in member:
                                member.remove(interaction.user)
                                embed.set_field_at(3, name="عدد المشاركين", value=len(member), inline=False)
                                await interaction.response.send_message("لقد خرجت من الجيف اوي", ephemeral=True)
                                cr_member.execute(f"delete from member_give where members_id = {interaction.user.id}")
                                db_member.commit()
                            else:
                                await interaction.response.send_message("انت خارج الجيف اوي بالفعل", ephemeral=True)
                        remove_button.callback = remove_callback
                        await interaction.response.send_message(f"لقد انضممت للجيف اوي بالفعل يا {interaction.user.mention} هل تريد الخروج ؟",view=remove_view, ephemeral=True)
...
async def member_join(interaction: discord.Interaction):
    ...
    try:
        ...
        if role in interaction.user.roles:
            cr_member.execute("select members_id from member_give")
